...
export default function TaskList({ user_id, isUnassigned, allowEdit }) {
    ...
    const handleBtnClick = async (task, isAccepting) => {
        ...
        const remainingTasks = tasks.filter(
            (_task) => _task.status_id != task.status_id
        );
    ...
    const handleCheck = async (_task) => {
        ...
        await updateUserCoins({
            ...user,
            coins: !_task.completed
                ? user.coins + _task.coins
                : user.coins - _task.coins,
        });
        ...
        <>
                    ...
                    .map((task) => (
                        <>
                            <Task
                                ...
                                handleBtnClick={() =>
                                    isUnassigned
                                        ? handleBtnClick(task, true)
                                        : handleBtnClick(task, false)
                                }
                            ...
                            >
                                <TaskDetail
                                    task={task}
                                    handleCloseBtn={() => handleCloseBtn(task)}
                                />
                            </section>
                        </>
                    ))
            ...
            {!isUnassigned && (
                <section className="completed">
                    <p className="container-header">Completed Tasks</p>
                    {tasks.length == 0 ? (
                        <p>No tasks yet.</p>
                    ) : (
                        tasks
            ...
            >
                {allowEdit && !isUnassigned ? (
                    <button className="btn">See pending tasks</button>
                ) : (
                    ""
                )}
            </Link>
